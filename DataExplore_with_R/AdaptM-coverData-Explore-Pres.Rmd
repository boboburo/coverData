...
* columns 11-14 are a binary representation of **wilderness_Area**. *(4 columns are mutually exlusive)*
...
The binary columns are mutually exclusive, in that, for each row only only of the four/forthy columns can contain a 1. For the purpose of exploratory analysis the two sets of binary columns **wildernessArea** and **soilType** are returned to their original form based on information in the *coyType.info* metadata file. 

In the *covtype.info* file there is extra meta data associated with the **soilType** column that is not in the original dataset. Each **soilType** has an unique associated 4-digit *ELU.Code*. The first digit of the *ELU.Code* represents its' **Climatic Zone** and the second digit its' *Geological Zone*. This extra information is introduced for exploratory analysis. 
...
* There are `r nrow(coverData)` rows across `r ncol(coverData)` columns. A summary is presented in `r citetab("sumTab")`. 
...
**coverType** is not evenly distributed. Two classes *(1.SpruceFir, 2.LodgepolePine)* comprimise 85% of all rows. `r citefig("countCT")` displays a bar graphy of **coverType**.
...
- **HS** distributions display left skew, normal, left skew as expected. 
...
The 10 numeric features are also examined in conjuction with the 7 levels of the **covType** varaible. 
...
The scatter plot reveals some interesting pattenrs 

- The hillshade at noon and 9am creates an elipsoid
- As the horizontal distance to a hydro increases, the variance in vertical distance increases. 
- As slope increase a proportion othe data's hillshade at 9am decreases, probably due to the aspect.
- *HD.Fire* has a sigmoid relationship with *Aspect*
- Similarily *Aspect* and *HS.noon* have a more difined sigmoid relationship. 
...
The feature importance of the 15 columns, with respect to *coverType* variable is calculated and shown in `r citefig('featureImportTab'). As expected **Elevation** is the top feature. For the *chi-square* measure two of the introduced features, **climaticZone** and **geologicZone** also are in the top 5. *(This may be due to incorrect bucketing of the nuermcical attributes)*

**Elevation** is also the top feature for *random.forest* feature importance. However after that,  numerical features are more favoured in contrast to the order for **chi.square** measures. 
